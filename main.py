...
def create_embed(stats, guild_name):
    ...
    embed = discord.Embed(title=f"📊 Прогресс: {guild_name}", color=0x3498DB)
    if lvl >= MAX_LEVEL:
        embed.add_field(name=f"Уровень {lvl} (MAX)", value="🟦" * 15 + " **100%**", inline=False)
    else:
        ...
        embed.add_field(name=f"Уровень: {lvl}", value=f"{bar} **{int(progress*100)}%**", inline=False)
...
@bot.tree.command(name="board", description="Статус уровня сервера")
