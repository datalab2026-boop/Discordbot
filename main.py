...
def create_embed(stats, guild_name):
    ...
    if lvl >= MAX_LEVEL:
        ...
        # Simple Calculation: Current / Goal
...
async def expadd(interaction: discord.Interaction, amount: int):
    ...
    save_data(data)
    await interaction.response.send_message(f"✅ Added {amount:,} EXP. Level: {stats['level']}", ephemeral=True)
...
async def expremove(interaction: discord.Interaction, amount: int):
    ...
    stats["level"] = sync_level(stats["current"])
    save_data(data)
    await interaction.response.send_message(f"🔻 Removed {amount:,} EXP. Level: {stats['level']}", ephemeral=True)
    await refresh_board(interaction.guild)
